@extends('master')
@section('content')
<body>
        <!--gelombang image-->
        <div class="relative">
            <h1 class="absolute my-8 mx-8 md:text-5xl text-white font-bold" style="font-family: Poppins;"> Tentang Kami </h1>
            <img class="md:w-1/2 " src="{{ asset('/asset/abstract5.png') }}" alt="">
            <img class="md:w-1/2 absolute top-0" src="{{ asset('/asset/line1.png') }}" alt="">
            <img class="md:w-1/2 absolute top-0" src="{{ asset('/asset/line2.png') }}" alt="">
        </div>

        <!--main konten-->
        <div class="grid grid-cols-2"> 
        <!--kolom 1-->
            <div class="flex"> 
                <img class="md:w-[80%] md:mx-16 pb-10" src="{{ asset('/asset/ilustrasi.png') }}" alt="">
            </div>
        <!--kolom 2-->
            <div class="flex">
                <h3 class="mx-10 md:text-lg" style="font-family: Poppins;"> Lorem ipsum, dolor sit amet consectetur adipisicing elit. Asperiores distinctio explicabo doloremque reiciendis sed, libero molestias. 
                    Hic adipisci voluptas debitis ducimus. Ipsa, quidem. Ea repellendus magnam aperiam ut consequatur exercitationem. </h3>   
            </div>
        </div> 

        <!--gelombang image-->
        <div class="relative">
            <img class="md:w-1/2 absolute bottom-0 right-0" src="{{ asset('/asset/abstract4.png') }}" alt="">
            <img class="md:w-1/2 absolute bottom-0 right-0" src="{{ asset('/asset/line34.png') }}" alt="">
            
        </div>




            <!-- Card 2 -->
            {{-- <div class="w-full md:w-1/2 p-4">
            <svg width="95" height="95" viewBox="0 0 95 95" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
            <circle cx="47.5" cy="47.5" r="47.5" fill="#F5F5FA"/>
            <rect x="5" y="6" width="78" height="78" fill="url(#pattern0)"/>
            <defs>
            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
            <use xlink:href="#image0_1271_2794" transform="scale(0.0078125)"/>
            </pattern>
            <image id="image0_1271_2794" width="128" height="128" xlink:href="data:image/png;base64,"/>
            </defs>
            </svg> --}}

            {{-- <div class="rounded-lg shadow-lg p-6 ">
                <div class="bg-white rounded-lg shadow-lg p-6 mr-0 text-left">
                    <div class="w-1/2">
                        <h2 class="text-xl font-semibold text-center mb-2">Visi dan Misi</h2>
                        <br>
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Molestiae aut, tenetur nulla explicabo qui incidunt nostrum reiciendis ea. Voluptatum doloribus corporis fugit inventore ullam temporibus eos nihil tempora officiis vero!
          
         
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Molestiae aut, tenetur nulla explicabo qui incidunt nostrum reiciendis ea. Voluptatum doloribus corporis fugit inventore ullam temporibus eos nihil tempora officiis vero!
          
          
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Molestiae aut, tenetur nulla explicabo qui incidunt nostrum reiciendis ea. Voluptatum doloribus corporis fugit inventore ullam temporibus eos nihil tempora officiis vero!
            </br>
                                </div>
            </div>
                            </div>
          
    </div> --}}
    
@endsection